...
# Add this after detect_language function
def tokenize_text(text, language="en"):
    ...
    # Japanese tokenization
        ...
        # In a production system, you might want to use a dedicated
        # Japanese tokenizer like fugashi or MeCab
        return re.findall(r'\w+|[^\s\w]', text)
    
    # For other languages, use NLTK's word_tokenize
    try:
        return word_tokenize(text, language=language if language != "zh-cn" else "english")
    except:
        # Fallback to simple regex-based tokenization
        return re.findall(r'\w+|[^\s\w]', text)

def segment_text_by_tokens(tokens, words_per_segment):
    """
    Segment a list of tokens into chunks, respecting punctuation boundaries.
    """
    segments = []
    current_segment = []
    current_count = 0
    ...
    for token in tokens:
        ...
        # Increment count if not just punctuation
        if token not in punctuation_marks:
            current_count += 1
        
        # Check if we should end the segment
        if current_count >= words_per_segment or token in ['.', '!', '?', '。', '！', '？']:
            # End segment at punctuation
            segments.append(''.join(current_segment))
            current_segment = []
            current_count = 0
        elif token in [',', ';', ':', '，', '；', '：'] and current_count > words_per_segment // 2:
            # End segment at minor punctuation if we're at least halfway to our target
            segments.append(''.join(current_segment))
            current_segment = []
            current_count = 0
...
def segment_text_smartly(text, language, words_per_segment):
    """
    Segment text intelligently based on language, tokens, and punctuation.
    Returns a list of text segments.
    """
    # First tokenize the text based on language
    tokens = tokenize_text(text, language)
    
    # Then segment tokens respecting natural boundaries
    return segment_text_by_tokens(tokens, words_per_segment)
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                # Use intelligent segmentation based on tokenization and punctuation
                segments = segment_text_smartly(text, language, words_per_segment)
                
                # If no segments were created or text is very short, use the whole text
                if not segments:
                    segments = [text]
                
                print(f"Split text into {len(segments)} segments with smart segmentation")
                ...
                for i, segment in enumerate(segments):
                    ...
                    # Check for silent audio before any processing
                    if np.max(np.abs(wav)) < 0.01:
                        print(f"Warning: Silent audio detected for segment {i+1} before processing")
                    
                    # Store original audio before processing
                    original_wav = wav.copy()
                    
                    # Process audio for smooth transitions with conservative settings
                    is_start = (i == 0)
                    is_end = (i == len(segments) - 1)
                    wav = process_audio_for_smooth_transition(wav, is_start, is_end)
                    
                    # Double-check that processing didn't silence the audio
                    if np.max(np.abs(wav)) < 0.01 and np.max(np.abs(original_wav)) > 0.01:
                        print(f"Warning: Processing created silent audio for segment {i+1}, reverting to original")
                        wav = original_wav
...
async def save_audio_file_background(wav_array, wav_path, opus_path, audio_format):
    try:
        ...
        if audio_format == "opus":
            # Use FFmpeg to convert WAV to Opus using clean settings
            subprocess.run(
                [
                    "ffmpeg", 
                    "-i", wav_path,
...
async def save_segment_audio_background(wav_array, segment_path, audio_format):
    try:
        ...
        # Check if audio is silent before saving
        if np.max(np.abs(wav_array.astype(np.float32) / 32767.0)) < 0.01:
            print(f"Warning: Attempting to save silent audio to {segment_path}")
        ...
        if audio_format == "wav":
            ...
            for segment_file in valid_segment_files:
                try:
                    ...
                    if np.max(np.abs(segment_float)) < 0.01:
                        print(f"Warning: Silent segment detected: {segment_file}, skipping")
                        continue
                    
                    # Process for the first segment
                    if combined_audio is None:
                        combined_audio = segment_float
                    else:
                        # Simple concatenation - safer than complex processing
                        combined_audio = np.concatenate([combined_audio, segment_float])
                except Exception as e:
                    print(f"Error reading segment {segment_file}: {e}")
            
            # Save the combined audio
            if combined_audio is not None:
                # Make sure combined audio isn't silent
                if np.max(np.abs(combined_audio)) < 0.01:
                    print("Warning: Combined audio is silent, something went wrong")
                else:
...
# Add these functions for smooth audio transitions
def crossfade_audio(audio1, audio2, overlap_samples=100):
    """
    Create a crossfade between two audio segments to make the transition smoother.
    Uses a smaller overlap and ensures audio isn't silenced.
    """
    # Make sure we have arrays with proper amplitude
    if np.max(np.abs(audio1)) < 0.01 or np.max(np.abs(audio2)) < 0.01:
        # One of the arrays is too quiet, just concatenate
    ...
    # Create the crossfade with smaller overlap
    audio1_end = audio1[-overlap_samples:]
    audio2_start = audio2[:overlap_samples]
    ...
    # Return the concatenated audio with crossfade
    return np.concatenate([audio1[:-overlap_samples], crossfade, audio2[overlap_samples:]])

def process_audio_for_smooth_transition(wav_array, is_start=False, is_end=False):
    """
    Process audio segment to ensure smooth transitions between segments.
    - Apply gentle fade in/out without affecting the main audio content
    """
    # Convert to float if not already
    if wav_array.dtype != np.float32:
        wav_array = wav_array.astype(np.float32) / 32767.0
    
    # Make a copy to avoid modifying the original array
    processed = wav_array.copy()
    
    # Only apply gentle fade-in at start (if not the first segment)
    if not is_start:
        fade_in_length = min(100, len(processed) // 10)  # Shorter fade, max 100 samples
        if fade_in_length > 0:
            fade_in = np.linspace(0.8, 1.0, fade_in_length)  # Less dramatic fade
            processed[:fade_in_length] *= fade_in
    
    # Only apply gentle fade-out at end (if not the last segment)
    if not is_end:
        fade_out_length = min(100, len(processed) // 10)  # Shorter fade, max 100 samples
        if fade_out_length > 0:
            fade_out = np.linspace(1.0, 0.8, fade_out_length)  # Less dramatic fade
            processed[-fade_out_length:] *= fade_out
    
    # Check if we've accidentally made it too quiet
    if np.max(np.abs(processed)) < 0.01 and np.max(np.abs(wav_array)) > 0.01:
        print("Warning: Audio processing made the segment too quiet, using original audio")
        return wav_array
